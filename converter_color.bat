...
for %%A in (") do set "ESC=%%~A"
:: ESC character (0x1B) - can't type directly, use for loop trick

:: Enable VT100 / virtual terminal processing
if not defined _VT_ENABLED (
    set "_VT_ENABLED=1"
    rem Enable ANSI on Win10+ via kernel32
    powershell -NoProfile -Command "$p=[char]27;$mode=8;if([Environment]::OSVersion.Version.Major -ge 10){$h=Add-Type -MemberDefinition '[DllImport(\"kernel32.dll\")]public static extern bool SetConsoleMode(IntPtr h,int m);' -Name 'K' -Namespace 'W' -PassThru;$handle=(Get-Process -Id $PID).Handle;$h::SetConsoleMode($handle,$mode)}" 2>nul
)
...
set "A_BG_GREEN=[42m"
set "A_BG_RED=[41m"
set "A_BOLD_CYAN=[1;36m"
set "A_BOLD_GREEN=[1;32m"
set "A_BOLD_YELLOW=[1;33m"
set "A_BOLD_RED=[1;31m"
set "A_BOLD_BLUE=[1;34m"
set "A_BOLD_WHITE=[1;37m"
