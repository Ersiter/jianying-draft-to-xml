...
| 位置 / 缩放 / 旋转 / 透明度 | ✓ | ✓ |
| 裁剪 (source_timerange) | ✓ | ✓ |
| 倍速播放 | ✓ timeremap/timeMap | ✓ 完整曲线 |
| 音量 | ✓ | ✓ |
| 静音 | ✓ | ✓ |
| 文字轨道 | ✓ generatoritem + SRT/ASS | ✓ |
| 文字样式 (字体/大小/颜色/描边/对齐) | ✓ generatoritem | ✓ |
| 文字入场/出场动画 | ✓ Animation In/Out | ✓ |
| 贴纸轨道 | ✗ | ✓ |
| 滤镜 (命名/效果) | ✓ filter | ✓ |
| 调色 (16项调节效果) | ✓ colorcorrector3way | ✓ |
| 转场 | ⚠️ 已实现，实测未生效 | ✓ |
| 关键帧动画 | ⚠️ 已实现，实测未生效 | ✓ |
| 画面翻转 | ✓ 负 scale | ✓ |
| 音频淡入淡出 | ✓ audiofade | ✓ |
...
1. **关键帧动画** — 代码已实现，实测未生效，需进一步调试
2. **转场** — 代码已实现，实测未生效，需进一步调试
3. **贴纸** — FCP7 XML 格式支持，尚未验证草稿数据映射
4. **曲线变速** — timeMap 格式符合 FCP7 规范，达芬奇导入支持待验证
5. **达芬奇 FCP7 导入不保留调色参数** — 达芬奇自身导出的 FCP7 XML 重导入后调色参数也丢失
6. **素材路径** — XML 引用原始绝对路径。素材移动后需在达芬奇中手动 Relink
